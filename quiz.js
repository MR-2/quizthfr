...
function initialize()
{
    ...
    var userInput = prompt('Combien équipes?', '2');
...
function selectItem(category, row, dd)
{
	...
	if (DailyDouble && dd==1)
		wager = prompt('Aujourd´hui DOUBLE!!!\nCombien veux tu pari??', wagerAmount);
...
{
    var title = "Final sujet: " + FinalCategory;

    var body =  "<table id='finalwager'>";
    body += "<tr><th>Equipe</th><th>Pari</th><th>Courant $</th></tr>";
    for (var i=1; i<=teamCount; i++)
    {
        var money = parseInt($("#team" + i).html());
        body += "<tr>";
        body += "<td align=center> Equipe " + i + "</th>";
...
{
    html = "<table> <tr> <td> <b>CORRECT REPONDRE</b> </td>";
    for (var i = 1; i<=teamCount; i++)
    {
        btnid = "btnright"+i;
        html += "<td>";
        html += "<input class='btnRight' type='button' id='" + btnid + 
                "'value='Equipe " + i + "' onclick='incScore(" + i + ");' />";
        html += "</td>";
    }
    html += "<td rowspan='2'> &nbsp;&nbsp;&nbsp;<input type='button' onclick='closeWindow();' value='Tableau de jeu' /> </td>";
    html += "</tr><tr><td><b>INCORRECT REPONDRE</b></td>";
    ...
    for (var i = 1; i<=teamCount; i++)
    {
        ...
        html += "<input class='btnWrong' type='button' id='" + btnid + 
                "' value='Equipe " + i + "' onclick='decScore(" + i + ");' />";
        html += "</td>";
    }
    html += "</tr></table>";  
    return html;  
...
function makeScoreTable()
{
    ...
    for (var i = 1; i<=teamCount; i++)
    {
        html += "<th contenteditable align=center> Equipe    " + i + "</th>";
    }
    ...
    {
        html += "<th rowspan='2'>";
        html += "<input type='button' value='Final Repondre' onclick='getFinalWagers();' />";
        html += "</th>";
    }
